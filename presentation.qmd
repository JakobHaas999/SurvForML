...
- This partitions the observations into two child nodes
- A good split seperates observations with different survival distributions
...
## Survival Trees: Log-rank Splitting
- Remember the log-rank statistic [@ishwaran2007random]:
$$
...
::: {.fragment}
![](results/logrank_splits_tumor.png){width=100%}
:::
...
- Inputs:
    ...
    - $d_0 \geq 0:$ Minimum number events in each terminal node
...
Grow the tree under the condition that each terminal node must have $d_0 \geq 0$ events.
...
- One possible RSF risk score is the ensemble ensemble cumulative hazard function
